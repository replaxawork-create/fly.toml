/* ============================================
   ♾️ INFINITY BOT V9 — SOLO WARRIOR
...
> ${PREFIX}ncbaap9 <text>
> ${PREFIX}stopnc9
> ${PREFIX}ncmaximum9 <text>

━━━━━━━━━━━━━━━━━━━━━━━
🔒 *NAME LOCK*
> ${PREFIX}namelock9 <text>
> ${PREFIX}stopnamelock9
...
async function startBot() {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(`./auth_bot_9`);
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (!_pairCodeRequested && !sock.authState.creds.registered && connection === "connecting") {
      ...
      const doLogin = async (phone) => {
        ...
        try {
          ...
          console.log(`\n╔══════════════════════════════╗`);
          console.log(`║  🔑  INFINITY V9 PAIR CODE       ║`);
          console.log(`╠══════════════════════════════╣`);
          console.log(`║        ${fmt}          ║`);
          console.log(`╚══════════════════════════════╝`);
    ...
    if (connection === "open") {
      log(`✅ GEAR 1 ONLINE`);
      try { fs.writeFileSync("./data/gear9_online.flag", "1"); } catch {}
    }
    if (connection === "close") {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // HELP
        if (["help9","start9","menu9"].includes(cmdName)) {
        ...
        // PREFIX
        if (isCmd(text, "prefix9")) {
          const np = getArg(text).trim();
          if (!np) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}prefix9 <new>` }, { quoted: msg }); continue; }
          PREFIX = np; saveSettings();
          await sock.sendMessage(chatId, { text: `✅ Prefix: *${PREFIX}*` }, { quoted: msg }); continue;
        }

        // SUDO MANAGEMENT
        if (isCmd(text, "addsudo9")) {
        ...
        if (isCmd(text, "listsudo9")) {
          await sock.sendMessage(chatId, {
            text: `👑 *Sudo:*\n> ${[...SUDO_USERS].map(u => `@${u.split("@")[0]}`).join("\n> ") || "None"}`
          }, { quoted: msg }); continue;
        }
        if (isCmd(text, "addinfinitesudo9")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "delinfinitesudo9")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "control")) {
          ...
          await sock.sendMessage(chatId, {
            text:
              `♾️ *INFINITY CONTROL PANEL — GEAR 9*\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `⚙️ *BOT*\n` +
        ...
        // ── NAME CHANGERS ──
        if (isCmd(text, "ncemo9")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncemo9 <text>` }, { quoted: msg }); continue; }
          startNCEMO(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncemo9 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncbaap9")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncbaap9 <text>` }, { quoted: msg }); continue; }
          startNCBAAP(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncbaap9 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnc9")) {
          const ok = _stopNC(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnc9 done ✅" : "❌ No active NC" }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncmaximum9")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncmaximum9 <text>` }, { quoted: msg }); continue; }
        ...
        // ── NAME LOCK ──
        if (isCmd(text, "namelock9")) {
          const base = getArg(text).trim();
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}namelock9 <text>` }, { quoted: msg }); continue; }
          startNameLock(sock, chatId, base, INFINITY_TEXTS);
          await sock.sendMessage(chatId, { text: `namelock9 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnamelock9")) {
          const ok = stopNameLock(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnamelock9 done ✅" : "❌ No active Name Lock" }, { quoted: msg }); continue;
        }

        // STOP ALL
        if (isCmd(text, "stopall9")) {
          stopAll(chatId);
          writeStopAllFlag(chatId);
          await sock.sendMessage(chatId, { text: "stopall9 done ✅" }, { quoted: msg }); continue;
        }
...
/* ============ START ============ */
console.log(`\n╔══════════════════════════════════╗`);
console.log(`║   ♾️  INFINITY BOT V9           ║`);
console.log(`║      SOLO WARRIOR ENGINE         ║`);
console.log(`╚══════════════════════════════════╝\n`);
saveBotCount(1);
/* ── TELEGRAM COMMAND RECEIVER ── */
setInterval(() => {
  const cmdFile = `./data/tg_cmd_gear9.json`;
  ...
  try {
    ...
    if (cmd === "status") {
      ...
      fs.writeFileSync(`./data/tg_status_gear9.json`, JSON.stringify(statusData));
    }
    ...
    if (cmd === "allgc" && extra) {
      ...
      (async () => {
        try {
          ...
          const resultFile = `./data/tg_allgc_result_gear9.json`;
          fs.writeFileSync(resultFile, JSON.stringify({ gear: 9, count, ncCmd, base }));
        } catch(e) {
          fs.writeFileSync(`./data/tg_allgc_result_gear9.json`, JSON.stringify({ gear: 9, count: 0, error: String(e) }));
        }
      })();
    }
  } catch (e) {}
