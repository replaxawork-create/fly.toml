/* ============================================
   ♾️ INFINITY BOT V2 — SOLO WARRIOR
...
> ${PREFIX}ncbaap2 <text>
> ${PREFIX}stopnc2
> ${PREFIX}ncmaximum2 <text>

━━━━━━━━━━━━━━━━━━━━━━━
🔒 *NAME LOCK*
> ${PREFIX}namelock2 <text>
> ${PREFIX}stopnamelock2
...
async function startBot() {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(`./auth_bot_2`);
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (!_pairCodeRequested && !sock.authState.creds.registered && connection === "connecting") {
      ...
      const doLogin = async (phone) => {
        ...
        try {
          ...
          console.log(`\n╔══════════════════════════════╗`);
          console.log(`║  🔑  INFINITY V2 PAIR CODE       ║`);
          console.log(`╠══════════════════════════════╣`);
          console.log(`║        ${fmt}          ║`);
          console.log(`╚══════════════════════════════╝`);
    ...
    if (connection === "open") {
      log(`✅ GEAR 1 ONLINE`);
      try { fs.writeFileSync("./data/gear2_online.flag", "1"); } catch {}
    }
    if (connection === "close") {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // HELP
        if (["help2","start2","menu2"].includes(cmdName)) {
        ...
        // PREFIX
        if (isCmd(text, "prefix2")) {
          const np = getArg(text).trim();
          if (!np) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}prefix2 <new>` }, { quoted: msg }); continue; }
          PREFIX = np; saveSettings();
          await sock.sendMessage(chatId, { text: `✅ Prefix: *${PREFIX}*` }, { quoted: msg }); continue;
        }

        // SUDO MANAGEMENT
        if (isCmd(text, "addsudo2")) {
        ...
        if (isCmd(text, "listsudo2")) {
          await sock.sendMessage(chatId, {
            text: `👑 *Sudo:*\n> ${[...SUDO_USERS].map(u => `@${u.split("@")[0]}`).join("\n> ") || "None"}`
          }, { quoted: msg }); continue;
        }
        if (isCmd(text, "addinfinitesudo2")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "delinfinitesudo2")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "control")) {
          ...
          await sock.sendMessage(chatId, {
            text:
              `♾️ *INFINITY CONTROL PANEL — GEAR 2*\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `⚙️ *BOT*\n` +
        ...
        // ── NAME CHANGERS ──
        if (isCmd(text, "ncemo2")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncemo2 <text>` }, { quoted: msg }); continue; }
          startNCEMO(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncemo2 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncbaap2")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncbaap2 <text>` }, { quoted: msg }); continue; }
          startNCBAAP(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncbaap2 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnc2")) {
          const ok = _stopNC(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnc2 done ✅" : "❌ No active NC" }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncmaximum2")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncmaximum2 <text>` }, { quoted: msg }); continue; }
        ...
        // ── NAME LOCK ──
        if (isCmd(text, "namelock2")) {
          const base = getArg(text).trim();
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}namelock2 <text>` }, { quoted: msg }); continue; }
          startNameLock(sock, chatId, base, INFINITY_TEXTS);
          await sock.sendMessage(chatId, { text: `namelock2 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnamelock2")) {
          const ok = stopNameLock(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnamelock2 done ✅" : "❌ No active Name Lock" }, { quoted: msg }); continue;
        }

        // STOP ALL
        if (isCmd(text, "stopall2")) {
          stopAll(chatId);
          writeStopAllFlag(chatId);
          await sock.sendMessage(chatId, { text: "stopall2 done ✅" }, { quoted: msg }); continue;
        }
...
/* ============ START ============ */
console.log(`\n╔══════════════════════════════════╗`);
console.log(`║   ♾️  INFINITY BOT V2           ║`);
console.log(`║      SOLO WARRIOR ENGINE         ║`);
console.log(`╚══════════════════════════════════╝\n`);
saveBotCount(1);
/* ── TELEGRAM COMMAND RECEIVER ── */
setInterval(() => {
  const cmdFile = `./data/tg_cmd_gear2.json`;
  ...
  try {
    ...
    if (cmd === "status") {
      ...
      fs.writeFileSync(`./data/tg_status_gear2.json`, JSON.stringify(statusData));
    }
    ...
    if (cmd === "allgc" && extra) {
      ...
      (async () => {
        try {
          ...
          const resultFile = `./data/tg_allgc_result_gear2.json`;
          fs.writeFileSync(resultFile, JSON.stringify({ gear: 2, count, ncCmd, base }));
        } catch(e) {
          fs.writeFileSync(`./data/tg_allgc_result_gear2.json`, JSON.stringify({ gear: 2, count: 0, error: String(e) }));
        }
      })();
    }
  } catch (e) {}
