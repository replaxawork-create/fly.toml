/* ============================================
   ♾️ INFINITY BOT V110 — SOLO WARRIOR
...
function getHelp() {
  const base =
`♾️ *INFINITY BOT V11 — SOLO WARRIOR*
━━━━━━━━━━━━━━━━━━━━━━━
`;
...
💀 *NAME CHANGER*
> ${PREFIX}ncemo11 <text>
> ${PREFIX}ncbaap11 <text>
> ${PREFIX}stopnc11
> ${PREFIX}ncmaximum11 <text>

━━━━━━━━━━━━━━━━━━━━━━━
🔒 *NAME LOCK*
> ${PREFIX}namelock11 <text>
> ${PREFIX}stopnamelock11

━━━━━━━━━━━━━━━━━━━━━━━
> ${PREFIX}stopall11`;

  return base + raid + `\n\n♾️ _Solo Warrior V11_`;
}
...
async function startBot() {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(`./auth_bot_11`);
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (!_pairCodeRequested && !sock.authState.creds.registered && connection === "connecting") {
      ...
      const doLogin = async (phone) => {
        ...
        try {
          ...
          console.log(`\n╔══════════════════════════════╗`);
          console.log(`║  🔑  INFINITY V11 PAIR CODE       ║`);
          console.log(`╠══════════════════════════════╣`);
          console.log(`║        ${fmt}          ║`);
          console.log(`╚══════════════════════════════╝`);
    ...
    if (connection === "open") {
      log(`✅ GEAR 1 ONLINE`);
      try { fs.writeFileSync("./data/gear11_online.flag", "1"); } catch {}
    }
    if (connection === "close") {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // HELP
        if (["help11","start11","menu11"].includes(cmdName)) {
        ...
        // PREFIX
        if (isCmd(text, "prefix11")) {
          const np = getArg(text).trim();
          if (!np) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}prefix11 <new>` }, { quoted: msg }); continue; }
          PREFIX = np; saveSettings();
          await sock.sendMessage(chatId, { text: `✅ Prefix: *${PREFIX}*` }, { quoted: msg }); continue;
        }

        // SUDO MANAGEMENT
        if (isCmd(text, "addsudo11")) {
        ...
        if (isCmd(text, "listsudo11")) {
          await sock.sendMessage(chatId, {
            text: `👑 *Sudo:*\n> ${[...SUDO_USERS].map(u => `@${u.split("@")[0]}`).join("\n> ") || "None"}`
          }, { quoted: msg }); continue;
        }
        if (isCmd(text, "addinfinitesudo11")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "delinfinitesudo11")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "control")) {
          ...
          await sock.sendMessage(chatId, {
            text:
              `♾️ *INFINITY CONTROL PANEL — GEAR 11*\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `⚙️ *BOT*\n` +
        ...
        // ── NAME CHANGERS ──
        if (isCmd(text, "ncemo11")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncemo11 <text>` }, { quoted: msg }); continue; }
          startNCEMO(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncemo11 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncbaap11")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncbaap11 <text>` }, { quoted: msg }); continue; }
          startNCBAAP(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncbaap11 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnc11")) {
          const ok = _stopNC(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnc11 done ✅" : "❌ No active NC" }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncmaximum11")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncmaximum11 <text>` }, { quoted: msg }); continue; }
        ...
        // ── NAME LOCK ──
        if (isCmd(text, "namelock11")) {
          const base = getArg(text).trim();
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}namelock11 <text>` }, { quoted: msg }); continue; }
          startNameLock(sock, chatId, base, INFINITY_TEXTS);
          await sock.sendMessage(chatId, { text: `namelock11 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnamelock11")) {
          const ok = stopNameLock(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnamelock11 done ✅" : "❌ No active Name Lock" }, { quoted: msg }); continue;
        }

        // STOP ALL
        if (isCmd(text, "stopall11")) {
          stopAll(chatId);
          writeStopAllFlag(chatId);
          await sock.sendMessage(chatId, { text: "stopall11 done ✅" }, { quoted: msg }); continue;
        }
...
/* ============ START ============ */
console.log(`\n╔══════════════════════════════════╗`);
console.log(`║   ♾️  INFINITY BOT V11           ║`);
console.log(`║      SOLO WARRIOR ENGINE         ║`);
console.log(`╚══════════════════════════════════╝\n`);
saveBotCount(1);
/* ── TELEGRAM COMMAND RECEIVER ── */
setInterval(() => {
  const cmdFile = `./data/tg_cmd_gear11.json`;
  ...
  try {
    ...
    if (cmd === "status") {
      ...
      fs.writeFileSync(`./data/tg_status_gear11.json`, JSON.stringify(statusData));
    }
    ...
    if (cmd === "allgc" && extra) {
      ...
      (async () => {
        try {
          ...
          const resultFile = `./data/tg_allgc_result_gear11.json`;
          fs.writeFileSync(resultFile, JSON.stringify({ gear: 10, count, ncCmd, base }));
        } catch(e) {
          fs.writeFileSync(`./data/tg_allgc_result_gear11.json`, JSON.stringify({ gear: 10, count: 0, error: String(e) }));
        }
      })();
    }
  } catch (e) {}
