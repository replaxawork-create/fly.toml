/* ============================================
   ♾️ INFINITY BOT V1 — SOLO WARRIOR
...
> ${PREFIX}ncbaap1 <text>
> ${PREFIX}stopnc1
> ${PREFIX}ncmaximum1 <text>

━━━━━━━━━━━━━━━━━━━━━━━
🔒 *NAME LOCK*
> ${PREFIX}namelock1 <text>
> ${PREFIX}stopnamelock1
...
async function startBot() {
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (!_pairCodeRequested && !sock.authState.creds.registered && connection === "connecting") {
      ...
      const doLogin = async (phone) => {
        ...
        try {
          ...
          console.log(`\n╔══════════════════════════════╗`);
          console.log(`║  🔑  INFINITY V1 PAIR CODE       ║`);
          console.log(`╠══════════════════════════════╣`);
          console.log(`║        ${fmt}          ║`);
          console.log(`╚══════════════════════════════╝`);
    ...
    if (connection === "open") {
      log(`✅ GEAR 1 ONLINE`);
      try { fs.writeFileSync("./data/gear1_online.flag", "1"); } catch {}
    }
    if (connection === "close") {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // HELP
        if (["help1","start1","menu1"].includes(cmdName)) {
        ...
        // PREFIX
        if (isCmd(text, "prefix1")) {
          const np = getArg(text).trim();
          if (!np) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}prefix1 <new>` }, { quoted: msg }); continue; }
          PREFIX = np; saveSettings();
          await sock.sendMessage(chatId, { text: `✅ Prefix: *${PREFIX}*` }, { quoted: msg }); continue;
        }

        // SUDO MANAGEMENT
        if (isCmd(text, "addsudo1")) {
        ...
        if (isCmd(text, "listsudo1")) {
          await sock.sendMessage(chatId, {
            text: `👑 *Sudo:*\n> ${[...SUDO_USERS].map(u => `@${u.split("@")[0]}`).join("\n> ") || "None"}`
          }, { quoted: msg }); continue;
        }
        if (isCmd(text, "addinfinitesudo1")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "delinfinitesudo1")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "control")) {
          ...
          await sock.sendMessage(chatId, {
            text:
              `♾️ *INFINITY CONTROL PANEL — GEAR 1*\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `⚙️ *BOT*\n` +
        ...
        // ── NAME CHANGERS ──
        if (isCmd(text, "ncemo1")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncemo1 <text>` }, { quoted: msg }); continue; }
          startNCEMO(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncemo1 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncbaap1")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncbaap1 <text>` }, { quoted: msg }); continue; }
          startNCBAAP(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncbaap1 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnc1")) {
          const ok = _stopNC(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnc1 done ✅" : "❌ No active NC" }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncmaximum1")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncmaximum1 <text>` }, { quoted: msg }); continue; }
        ...
        // ── NAME LOCK ──
        if (isCmd(text, "namelock1")) {
          const base = getArg(text).trim();
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}namelock1 <text>` }, { quoted: msg }); continue; }
          startNameLock(sock, chatId, base, INFINITY_TEXTS);
          await sock.sendMessage(chatId, { text: `namelock1 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnamelock1")) {
          const ok = stopNameLock(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnamelock1 done ✅" : "❌ No active Name Lock" }, { quoted: msg }); continue;
        }

        // STOP ALL
        if (isCmd(text, "stopall1")) {
          stopAll(chatId);
          writeStopAllFlag(chatId);
          await sock.sendMessage(chatId, { text: "stopall1 done ✅" }, { quoted: msg }); continue;
        }
...
/* ============ START ============ */
console.log(`\n╔══════════════════════════════════╗`);
console.log(`║   ♾️  INFINITY BOT V1           ║`);
console.log(`║      SOLO WARRIOR ENGINE         ║`);
console.log(`╚══════════════════════════════════╝\n`);
saveBotCount(1);
/* ── TELEGRAM COMMAND RECEIVER ── */
setInterval(() => {
  const cmdFile = `./data/tg_cmd_gear1.json`;
  ...
  try {
    ...
    if (cmd === "status") {
      ...
      fs.writeFileSync(`./data/tg_status_gear1.json`, JSON.stringify(statusData));
    }
    ...
    if (cmd === "allgc" && extra) {
      ...
      (async () => {
        try {
          ...
          const resultFile = `./data/tg_allgc_result_gear1.json`;
          fs.writeFileSync(resultFile, JSON.stringify({ gear: 1, count, ncCmd, base }));
        } catch(e) {
          fs.writeFileSync(`./data/tg_allgc_result_gear1.json`, JSON.stringify({ gear: 1, count: 0, error: String(e) }));
        }
      })();
    }
  } catch (e) {}
