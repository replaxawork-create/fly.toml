/* ============================================
   ♾️ INFINITY BOT V6 — SOLO WARRIOR
...
> ${PREFIX}ncbaap6 <text>
> ${PREFIX}stopnc6
> ${PREFIX}ncmaximum6 <text>

━━━━━━━━━━━━━━━━━━━━━━━
🔒 *NAME LOCK*
> ${PREFIX}namelock6 <text>
> ${PREFIX}stopnamelock6
...
async function startBot() {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(`./auth_bot_6`);
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (!_pairCodeRequested && !sock.authState.creds.registered && connection === "connecting") {
      ...
      const doLogin = async (phone) => {
        ...
        try {
          ...
          console.log(`\n╔══════════════════════════════╗`);
          console.log(`║  🔑  INFINITY V6 PAIR CODE       ║`);
          console.log(`╠══════════════════════════════╣`);
          console.log(`║        ${fmt}          ║`);
          console.log(`╚══════════════════════════════╝`);
    ...
    if (connection === "open") {
      log(`✅ GEAR 1 ONLINE`);
      try { fs.writeFileSync("./data/gear6_online.flag", "1"); } catch {}
    }
    if (connection === "close") {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // HELP
        if (["help6","start6","menu6"].includes(cmdName)) {
        ...
        // PREFIX
        if (isCmd(text, "prefix6")) {
          const np = getArg(text).trim();
          if (!np) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}prefix6 <new>` }, { quoted: msg }); continue; }
          PREFIX = np; saveSettings();
          await sock.sendMessage(chatId, { text: `✅ Prefix: *${PREFIX}*` }, { quoted: msg }); continue;
        }

        // SUDO MANAGEMENT
        if (isCmd(text, "addsudo6")) {
        ...
        if (isCmd(text, "listsudo6")) {
          await sock.sendMessage(chatId, {
            text: `👑 *Sudo:*\n> ${[...SUDO_USERS].map(u => `@${u.split("@")[0]}`).join("\n> ") || "None"}`
          }, { quoted: msg }); continue;
        }
        if (isCmd(text, "addinfinitesudo6")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "delinfinitesudo6")) {
          if (!isOwner(sender)) { await sock.sendMessage(chatId, { text: "❌ Only Owner." }, { quoted: msg }); continue; }
          const ctx = msg.message?.extendedTextMessage?.contextInfo;
          if (!ctx?.participant) { await sock.sendMessage(chatId, { text: "⚠️ Reply to a user." }, { quoted: msg }); continue; }
        ...
        if (isCmd(text, "control")) {
          ...
          await sock.sendMessage(chatId, {
            text:
              `♾️ *INFINITY CONTROL PANEL — GEAR 6*\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `⚙️ *BOT*\n` +
        ...
        // ── NAME CHANGERS ──
        if (isCmd(text, "ncemo6")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncemo6 <text>` }, { quoted: msg }); continue; }
          startNCEMO(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncemo6 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncbaap6")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncbaap6 <text>` }, { quoted: msg }); continue; }
          startNCBAAP(sock, chatId, base);
          await sock.sendMessage(chatId, { text: `ncbaap6 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnc6")) {
          const ok = _stopNC(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnc6 done ✅" : "❌ No active NC" }, { quoted: msg }); continue;
        }
        if (isCmd(text, "ncmaximum6")) {
          const base = getArg(text);
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}ncmaximum6 <text>` }, { quoted: msg }); continue; }
        ...
        // ── NAME LOCK ──
        if (isCmd(text, "namelock6")) {
          const base = getArg(text).trim();
          if (!base) { await sock.sendMessage(chatId, { text: `⚠️ ${PREFIX}namelock6 <text>` }, { quoted: msg }); continue; }
          startNameLock(sock, chatId, base, INFINITY_TEXTS);
          await sock.sendMessage(chatId, { text: `namelock6 started ✅` }, { quoted: msg }); continue;
        }
        if (isCmd(text, "stopnamelock6")) {
          const ok = stopNameLock(chatId);
          await sock.sendMessage(chatId, { text: ok ? "stopnamelock6 done ✅" : "❌ No active Name Lock" }, { quoted: msg }); continue;
        }

        // STOP ALL
        if (isCmd(text, "stopall6")) {
          stopAll(chatId);
          writeStopAllFlag(chatId);
          await sock.sendMessage(chatId, { text: "stopall6 done ✅" }, { quoted: msg }); continue;
        }
...
/* ============ START ============ */
console.log(`\n╔══════════════════════════════════╗`);
console.log(`║   ♾️  INFINITY BOT V6           ║`);
console.log(`║      SOLO WARRIOR ENGINE         ║`);
console.log(`╚══════════════════════════════════╝\n`);
saveBotCount(1);
/* ── TELEGRAM COMMAND RECEIVER ── */
setInterval(() => {
  const cmdFile = `./data/tg_cmd_gear6.json`;
  ...
  try {
    ...
    if (cmd === "status") {
      ...
      fs.writeFileSync(`./data/tg_status_gear6.json`, JSON.stringify(statusData));
    }
    ...
    if (cmd === "allgc" && extra) {
      ...
      (async () => {
        try {
          ...
          const resultFile = `./data/tg_allgc_result_gear6.json`;
          fs.writeFileSync(resultFile, JSON.stringify({ gear: 6, count, ncCmd, base }));
        } catch(e) {
          fs.writeFileSync(`./data/tg_allgc_result_gear6.json`, JSON.stringify({ gear: 6, count: 0, error: String(e) }));
        }
      })();
    }
  } catch (e) {}
